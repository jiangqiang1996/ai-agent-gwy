...
1. **意图识别**：优先判断用户是在要知识点总结、框架梳理、截图讲题、经典例题、导出、更新资料还是查看已有档案摘要。
...
| 截图题目讲解 | 截图工作流 → QuestionArtifact → 对应模块老师 + 共享规则选中的状元代理(可选) |
| 经典例题 / 代表性示例 | 对应模块老师 |
...
2. 若名字未使用，则调用 `loadOrCreate` 创建新档案，并补问：
   - 考试类型：国考 / 省考 / 事业单位（可多选，可跳过）
   - 身份：在职 / 应届生（可跳过）
   - 省份：在哪个省份参加考试（可跳过）
3. 若名字已存在，则引导用户在“加载已有档案 / 换名字 / 覆盖旧档案”之间选择
...
- 截图题目讲解遵循 `.opencode/rules/screenshot-question-workflow.md`
...
3. 截图题讲解时，必须先走截图工作流产出完整 `QuestionArtifact`，再交给老师解释；不完整或低置信度的截图必须回退要求用户补图或补文字，不得硬讲。
