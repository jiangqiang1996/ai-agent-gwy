...
| 题目讲解（图片/文字/文件） | 题目输入工作流 → QuestionArtifact → 确认门控 → 对应模块老师 + 共享规则选中的状元代理(可选) |
| 经典例题 / 代表性示例 | 对应模块老师 |
| 导出当前内容 | 按导出工作流规则确定格式 → 加载对应 export-html 或 export-markdown skill → 调用 `export-document` 工具 |
| 内联已有 HTML 资源 | 加载 `inline-html` skill → 调用 `inline-html-resources` 工具 |
...
3. 题目讲解时，无论输入来自图片、对话文字还是外部文件，必须先走题目输入工作流收敛为 `QuestionArtifact`，经确认门控后再交给老师解释；不完整或低置信度的题目必须走确认门控让用户确认，不得硬讲。
