...
将指定内容导出为精美 HTML 文件。支持数学公式、流程图、数据图表、SVG 图形、Canvas 动态绘图、思维导图、折叠展开、Tab 切换和自动目录。HTML 导出默认生成 HTML 文件 + 同级资源目录的完整包，本地图片和运行时资源以引用方式写入资源目录，导出前逐一验证可访问性。如需单文件离线包，请在导出后使用内联 HTML 资源功能。
...
- 远程图片会保留原 URL 并在导出时验证可达性；工作区内的本地图片复制到同级资源目录。
...
导出的 HTML 支持在试题区域上进行鼠标涂鸦（Scratchpad）：

- 在 content 中用 `<section data-exam-question>` 或 `<div data-exam-question>` 包裹题目内容即可启用涂鸦板。
...
- 只有显式导出意图时才落文件。
- 默认导出到项目根目录 `output/`。
- 导出结果为 HTML 文件 + 同级资源目录，移动时必须一起移动。
